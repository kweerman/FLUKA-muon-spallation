...
def submit_flukaruns(path, inp_file, copy_file, job_folder, out_folder, log_folder, python_filepath, fort_name='SRCEFILE', cycles=1):
    ...
    for cycle in range(1, cycles + 1):
        ...
        script1 = '$FLUPRO/flutil/rfluka -e $FLUPRO/flutil/XeLS_event_plotting -N0 -M1 ' + new_inp
        script2 = 'python {0}eventscreator_event_plotting.py {1} {2}'.format(python_filepath, userdump_file, event_file)

        # we move all necesarry output to the dcache folder
        move_line = 'mv {0} {1} {2} {3} {4} '.format(userdump_file, event_file, neutron_file, AZ_file, resid_file)
